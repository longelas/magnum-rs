...
fn example_ogg_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/tests/Example.ogg")
}

/// Opens Example.ogg as Opus-in-OGG. Skips (returns None) if the file is missing or not
/// Opus (e.g. Vorbis). Use an Opus-encoded OGG file at src/Example.ogg for playback tests.
fn open_example_opus_ogg() -> Option<OpusSourceOgg<BufReader<File>>> {
    let path = example_ogg_path();
    let file = File::open(&path).ok()?;
    let reader = BufReader::new(file);
    match OpusSourceOgg::new(reader) {
        Ok(source) => Some(source),
        Err(OpusSourceError::InvalidHeaderData) => {
            eprintln!("Skip: src/Example.ogg is not Opus-in-OGG (e.g. Vorbis). Use an Opus OGG file for tests.");
            None
        }
        Err(e) => panic!("Unexpected error opening Example.ogg: {}", e),
    }
}
...
#[test]
fn example_ogg_playback_produces_samples() {
...
#[test]
fn example_ogg_playback_consumes_full_stream() {
    let source = match open_example_opus_ogg() {
        Some(s) => s,
        None => return,
    };
...
fn rodio_source_trait_ogg() {
    ...
    assert_eq!(source.current_frame_len(), Some(240));
    assert!(source.total_duration().is_none());
}

#[cfg(feature = "with_rodio")]
#[test]
fn rodio_source_playback_ogg() {
    let mut source = match open_example_opus_ogg() {
        Some(s) => s,
        None => return,
    };
